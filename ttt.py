...
def getPlayerMove(board):
    ...
    while player_move not in '1 2 3 4 5 6 7 8 9'.split() or not isSpaceFree(board, int(player_move)):
...
def getComputerMove(board, opponentLetter): # TODO: W0621: Redefining name 'computerLetter' from outer scope. Hint: Fix it according to https://stackoverflow.com/a/25000042/81306
    ...
    if move: # TODO: Fix it (Hint: Comparisons to singletons like None should always be done with is or is not, never the equality/inequality operators.)
        return move
...
def player_turn(board, playerLetter):
    draw_board(board)
    move = getPlayerMove(theBoard)
    makeMove(theBoard, playerLetter, move)

def computer_turn(board, computerLetter):
    move = getComputerMove(theBoard, computerLetter)
    makeMove(theBoard, computerLetter, move)

def setUpBoard():
    theBoard = [' '] * NUMBER_OF_SPACES
    playerLetter, computerLetter = inputPlayerLetter()
    turn = whoGoesFirst()
    print('The ' + turn + ' will go first.')
    return theBoard, playerLetter, computerLetter, turn

def gameIsRunning(theBoard, playerLetter, computerLetter, turn):
    while True: # TODO: Usually (not always), loops (or their content) are good candidates to be extracted into their own function.
        ...
        if turn == 'player':
            # Player’s turn.
            player_turn(theBoard, playerLetter)
            ...
            # Computer’s turn.
            computer_turn(theBoard, computerLetter)
...
while True:
    # Reset the board
    theBoard, playerLetter, computerLetter, turn = setUpBoard()
    gameIsRunning(theBoard, playerLetter, computerLetter, turn)
    if not playAgain():
        break